...
const DeploymentCard = ({ deployment }) => {
  ...
  return (
    ...
    >
      <motion.div
        ref={nodeRef}
        className={`bg-app-card border border-app-border rounded-lg shadow-md w-48 h-40 select-none absolute z-10 cursor-move ${
          isDragging ? 'shadow-lg ring-2 ring-app-accent-purple' : ''
        }`}
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        transition={{ duration: 0.3 }}
